<li>
    <a href="{{ route('get.product.by_slug', ['slug' => $item->slug]) }}" class="product-detail-link d-flex position-relative">
        <div class="product-detail-thumbnail">
            <img src="{{ pare_url_file($item->avatar) ?? "#"}}" alt="{{ $item->name ?? "[N/A]"}}" width="100%" height="100%">
        </div>
        <div class="d-flex flex-column">
            <h3 class="product-detail-title">{{ $item->name ?? "[N/A]" }}</h3>   
            <p class="product-detail-price">{{ number_format($item->price, 0, ',', '.') ?? "[N/A]" }} đ</p> 
